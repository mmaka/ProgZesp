...
namespace dodawanie_figur1
{
    public class Akcje_widok
    {
        ...
        public void nowa()
        {
            ...
            okno.zapiszToolStripMenuItem.Enabled = true;
        ...
        public void zapiszObraz()
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                okno.textBox1.Text = "Zapisano pomyślnie";
            }
        }


        internal void zapiszTabelaPlik()
        {

            SaveFileDialog dialog = new SaveFileDialog();
            dialog.FileName = "tabela";
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                string lines = "";
                lines += "Nazwa: \t ID: \t: Pole:\t X:\t Y:\t W:\t H:\t";
                sW.WriteLine(lines);

                for (int row = 0; row < okno.dataGridView1.Rows.Count-1; row++)
                {
                  lines = "";
                 
                    for (int col = 0; col < okno.dataGridView1.ColumnCount; col++)
                    {
                        lines += (string.IsNullOrEmpty(lines) ? " " : "  ") + okno.dataGridView1.Rows[row].Cells[col].Value.ToString();
                    }
